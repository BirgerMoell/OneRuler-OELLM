...
## Scale down proportions to fit smaller context
if args.freq_cw == 30 and args.freq_ucw == 3:
    if args.max_seq_length <= 8000:
        args.freq_cw = 10
        args.freq_ucw = 1
elif args.freq_cw == 20 and args.freq_ucw == 10:
    if args.max_seq_length <= 8000:
        args.freq_cw = 10
        args.freq_ucw = 5
...
def sys_word_pair_random(num_samples: int, max_seq_length: int, save_dir: str, incremental: int = 1):
    ...
    print(f"[parameter] num_cw: {args.num_cw}, freq_cw: {args.freq_cw}, freq_ucw: {args.freq_ucw}, incremental: {incremental}")
    num_words = incremental
    total_tokens = 0  

    for index in tqdm(range(num_samples)):
        while total_tokens < max_seq_length:
            input_text, answer = generate_input_output(num_words)
            # Calculate the number of tokens in the example
            total_tokens = len(TOKENIZER.text_to_tokens(input_text))
            print(f'Max length {max_seq_length}| Current length {total_tokens} | Words: {num_words}')
            if total_tokens > max_seq_length:
                num_words -= incremental
                print(f"choose {num_words} words with {total_tokens} tokens")
                break
                
            num_words += incremental
            if num_words > len(words):
                num_words = len(words)
                print("run out of vocabs")
                break
